...
def validate_assets():
    disk = {f.name for f in ASSET_DIR.glob("*.mp4")}
    declared = set(ASSET_KEYWORDS.keys())

    if disk - declared:
        sys.exit(f"❌ Assets on disk NOT declared: {disk - declared}")
    if declared - disk:
        sys.exit(f"❌ Assets declared but missing on disk: {declared - disk}")
